...
mod tests {
    ...
    /// Join at T=0, sweep at T+8s: NOT reaped. Heartbeat resets timer,
    /// sweep at T+8s after heartbeat: NOT reaped.
    #[tokio::test(start_paused = true)]
    async fn test_heartbeat_resets_timer() {
        ...
        // Simulate 8s elapsed by injecting a future Instant
        let join_time = Instant::now();
        let after_8s = join_time + Duration::from_secs(8);

        // sweep_once at T+8s: elapsed = 8s < 15s — NOT reaped
        ...
        // Heartbeat resets last_heartbeat to Instant::now()
        registry.heartbeat(board_id, "u2");

        // Sweep at 8s after the heartbeat: still < 15s — NOT reaped
    ...
    async fn test_sweep_reaps_stale() {
        ...
        // Sweep with now = join_time + 16s (> 15s threshold)
        let join_time = Instant::now();
